...
This project demonstrates a Spec-Driven Development (SDD) workflow leveraging Google Antigravity and Gemini, although it is also possible to use other AI tools such as Cursor, Windsurf, or GitHub Copilot. The system automates the transition from high-level documentation to a fully functional Java-based application.
...
The main branch shows the final result after the Spec-Driven Development process is complete. If you want to see the "before" state (i.e., the 2–3 files used as a starting point), look at the [Starting_spec branch](https://github.com/paulbrowne-irl/Auto-generate_Business_Rules/tree/Starting_spec).
